...
class PXADrawControl(wx.Control):
	...
	def __init__(self, parent, imageSize=(64,64), color=(255,255,255,255), id=wx.ID_ANY,  pos=wx.DefaultPosition,
		...
		self.parent = parent
		self.image = wx.EmptyBitmap(imageSize[0], imageSize[1], 32)
	...
	def onPaint(self, event):
		...
		x = int((w / 2) - self.imageSize[0] / 2)
		y = int((h / 2) - self.imageSize[1] / 2)
		if x < 0:
			x = 0
		if y < 0:
			y = 0
	...
	def onLeftClick(self, event):
		...
		if (ww - iw) / 2 < x < ww / 2 + iw / 2 \
		  and (wh - iw) / 2 < y < wh / 2 + ih / 2:
			...
			ny = (wh - ih) / 2
			print nx, ny
			dc.DrawBitmap(self.image, nx, ny)
			self.Refresh(False)
...
class MainWindow(wx.Frame):
	def __init__(self, parent, title):
		...
		self.sizer.Add(PXAColorPicker(self), 0, wx.SHAPED)
